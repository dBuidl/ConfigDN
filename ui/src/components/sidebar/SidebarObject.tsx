...
export default function SidebarObject(props: { type: "team" | "project" | "config", url: string, name: string, children?: ComponentChildren }) {
    ...
    return <div>
        ...
        <div className={`flex items-center gap-2 rounded-xl px-3 py-2 transition hover:bg-panel ${indent}`}>
            <div className={`h-2 w-2 shrink-0 rounded-full ${color}`}/>
            <Link to={props.url} className={`min-w-0 overflow-hidden text-ellipsis whitespace-nowrap text-sm font-semibold ${props.type === "team" ? "text-copy" : "text-muted"}`}>{props.name}</Link>
        </div>
